...
class MapsEstudiante : AppCompatActivity() {
    private lateinit var mapa: GoogleMap
    var permisos = false

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_maps_estudiante)
        solicitarPermisos()
        var mapaEstudiante = intent.getSerializableExtra("estudiante") as BEstudiante
        ...
        fragmentoMapa.getMapAsync { googleMap ->
            if (googleMap != null) {
                ...
                val pelicula = LatLng(mapaEstudiante.latitud, mapaEstudiante.altitud)
                val titulo = mapaEstudiante.nombreEstudiante.toString()
                val zoom = 17f
                anadirMarcador(pelicula, titulo)
                moverCamaraConZoom(pelicula, zoom)
                // LINEA
